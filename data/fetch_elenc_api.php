...
foreach ($api_endpoints as $key => $url) {
    ...
    if ($response === false) {
        ...
        trigger_error("Curl error for $key: $error", E_USER_WARNING);
        $results[$key] = "error";
    } else {
        ...
        if (preg_match('/<span[^>]*>(.*?)<\/span>/s', $response, $matches)) {
            ...
            trigger_error("No valid data found for $key", E_USER_WARNING);
            $results[$key] = "error";
        }
    }
...
// store in file
if (file_put_contents('/home/users/dobbertin-elektronik/www/elnec_api/results.json', $json_result) === false) {
    trigger_error("Unable to save results to file results.json", E_USER_WARNING);
} else {
